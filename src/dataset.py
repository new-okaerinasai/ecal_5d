...
class MyDataset(object):
    ...
    def __init__(self, opt, phase, domain):
        ...
        aux_path = opt['%s_aux_%s_path' % (phase, domain)]
        img_size = opt['img_size_%s' % domain]
        img_dims = opt['img_channels_%s' % domain]

        # Process images
        if img_path[-3:] == 'txt':

            # Read images from the list
            data_img_path = opt['images_path']
            ...
            # Read images from numpy array
            self.data_img = np.load(img_path).astype('float32')

            # Insert channels dim if not present
            if len(self.data_img.shape) < 4:
                self.data_img = self.data_img[:, None]

            self.length = self.data_img.shape[0]
        ...
        # Process attributes
        if len(aux_path):
            self.data_aux = np.load(aux_path).astype('float32')
        else:
            self.data_aux = None
        ...
        if img_path[-3:] == 'npy':
            ...
            self.output_transform += [Lambda(lambda x: torch.from_numpy(x))]
    ...
    def get_item(self, index, flip=None):
        ...
        if not self.data_aux is None:
            aux = torch.from_numpy(self.data_aux[index].astype('float32'))
            return [img, aux]
        else:
            return [img]
...
class MyDatasets(Dataset):
    ...
    def __init__(self, opt, phase='train'):
        ...
        self.down = Resize(opt['img_size_A'], interpolation=Image.BICUBIC)
