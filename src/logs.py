...
class Logs(object):
    ...
    def __init__(self, model, opt):
        ...
        if hasattr(self.model, 'dis_A'):
            self.n_discs['A'] = len(self.model.dis_A.module)
        else:
            self.n_discs['A'] = 0
        
        if hasattr(self.model, 'dis_B'):
            self.n_discs['B'] = len(self.model.dis_B.module)
        else:
            self.n_discs['B'] = 0
    ...
    def update_tboard(self, n_iter):
        ...
        if hasattr(self.model, 'real_A'):
            self.model.real_A = self.up(self.model.real_A)
        if hasattr(self.model, 'fake_A'):
            self.model.fake_A = self.up(self.model.fake_A)
